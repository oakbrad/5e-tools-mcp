 ...
 * Uses lambdas so dice are rolled at call time, not at import time.
 */
...
const HOARD_TIER1: HoardTreasureEntry[] = [
  ...
  {
    d100: 70,
    coins: () => ({ cp: rollDice(2, 6), sp: rollDice(7, 6), ep: 0, gp: rollDice(4, 6), pp: 0 }),
    gems: () => generateGems(1, 10, 2),
  },
  {
    d100: 95,
    coins: () => ({ cp: rollDice(4, 6), sp: rollDice(5, 6), ep: 0, gp: rollDice(3, 6), pp: 0 }),
    gems: () => generateGems(1, 8, 3),
    magicTable: "F",
  },
  {
    d100: 100,
...
const HOARD_TIER3: HoardTreasureEntry[] = [
  ...
  {
    d100: 35,
    coins: () => ({ cp: rollDice(1, 6) * 100, sp: 0, ep: 0, gp: rollDice(1, 6) * 1000, pp: 0 }),
  },
  {
    d100: 75,
    coins: () => ({
      ...
      sp: 0,
      ep: rollDice(2, 6) * 100,
      gp: rollDice(1, 6) * 1000,
      pp: 0,
    }),
    gems: () => generateGems(3, 6, 7),
  },
  {
    d100: 95,
    coins: () => ({
...
const HOARD_TIER4: HoardTreasureEntry[] = [
  ...
  {
    d100: 55,
    coins: () => ({ cp: 0, sp: 0, ep: 0, gp: rollDice(12, 6) * 100, pp: rollDice(8, 6) * 10 }),
  },
  {
    d100: 100,
    coins: () => ({
      cp: 0,
      sp: 0,
      ep: rollDice(2, 6) * 1000,
      gp: rollDice(8, 6) * 100,
      pp: rollDice(4, 6) * 100,
    }),
    gems: () => generateGems(5, 8, 9),
    art: () => generateArt(4, 6, 10),
    magicTable: "I",
  },
];
...
const ART_DESCRIPTIONS: Record<number, string[]> = {
  ...
  2: [
    "Gold comb with gold filigree",
    "Silver pendant with gemstone",
    "Gold ring with jewel",
    "Brooch with gemstone",
  ],
  3: [
    "Gold cup with gemstones",
    "Silver necklace with gemstones",
    "Gold crown with jewels",
    "Jeweled bracelet",
  ],
  4: [
    "Gold statuette with gems",
    "Painted portrait with jeweled frame",
    "Gold chalice with gemstones",
    "Silver mask with gems",
  ],
  5: [
    "Golden throne with jewels",
    "Sculpture of precious metal with gems",
    "Gold bowl with gemstones",
    "Ancient tapestry with gold thread",
  ],
  6: [
    "Golden crown with many jewels",
    "Silver statue with gemstones",
    "Gold chest with jewels",
    "Enchanted painting",
  ],
  7: [
    "Golden throne with many jewels",
    "Silver chest with gemstones",
    "Golden crown with ancient jewels",
    "Jeweled crown of royalty",
  ],
  8: [
    "Golden throne with many jewels",
    "Silver chest with gemstones",
...
/** Safe fraction parsing for CR strings (replaces eval) */
 ...
 */
function rollMagicItemTable(
  tableLetter: string,
  magicItemPreference: "none" | "few" | "many",
): MagicItem | null {
...
export function generateIndividualTreasure(cr: number | string): TreasureHoard {
  ...
  const rollFn = INDIVIDUAL_TREASURE[normalizedCR];
  const coins = rollFn ? rollFn() : { cp: 0, sp: 0, ep: 0, gp: 0, pp: 0 };

  const totalValueGP = coins.cp * 0.01 + coins.sp * 0.1 + coins.ep * 0.5 + coins.gp + coins.pp * 10;

  const description = `Individual treasure for CR ${normalizedCR}: ${formatCoins(coins)}`;
  ...
  return {
    ...
    cr: parseCRToNumber(normalizedCR),
  };
}
...
export function generateHoardTreasure(
  ...
  if (magicItems.length > 0) {
    descriptionParts.push(
      `Magic Items: ${magicItems.map((m) => `${m.description} (${m.rarity})`).join(", ")}`,
    );
  }
